...
<<<<<<< HEAD
  ...
  # Swath-MS Analysis
  swath_files_raw <- eventReactive(input$swath_button,{
    res <- list()
    withProgress(message = 'Loading Data', value = 0.1, {
      for (i in seq_along(input$iso_files$datapath)){
        res[[i]] <- LoadSwath(input$iso_files$datapath[i])
        incProgress(1/length(input$iso_files$datapath))
      }
    })
    res
  ...
  swath_files_ms1 <- reactive({
    iso_raws()
  })
  
  swath_files_ms2 <- reactive({
    lapply(swath_files_raw, function(r){
      getSwathData(r, tar_mz)
    })
  })
  
  swath_ref_ms2 <- reactive({
    ind <- which(iso_name_sample() == input$iso_name_sample)
    swath_files_ms2()[[ind]]
  })
  
  swath_mz <- reactive({
    getSwathMz(swath_ref_ms2, input$iso_target_position, input$swath_ppm)
  })
  
  swath_files_ms1_eics <- reactive({
    lapply(swath_files_ms1, function(f){
      getIsoEIC.mz(f, tar_mz, nmax = 1, ppm = ppm)
    })
  })
  
  swath_files_ms2_eics <- reactive({
    withProgress(message = 'Generating EICs', value = 0.1, {
      lapply(swath_files_ms2, function(f){
        getSwathEICs(f, swath_mz, ppm)
      })
    })
  })
  
  swath_frag_corr <- reactive({
    ind <- which(iso_name_sample() == input$iso_name_sample)
