...
Actually, for the end user, all of the functions can be used in the shiny app. Here is some funtions invoked in the app. The MM14 dataset is used for an example, which can be download at **https://www.ebi.ac.uk/metabolights/MTBLS188**

```{r, eval=TRUE, cache=TRUE, message=FALSE, warning=FALSE}
# Load Dataset
library(TarMet)
files <- list.files('E:/MM48', full.names = TRUE)
files
...
eic <- getEIC(raw, rtrange = c(0, Inf), mzrange = c(288.05, 288.10))
plot(eic$rt, eic$intensity, 'l')

# But usually we usually get the isotopic EICs of a specific formula at the same time.
eics <- getIsoEIC(raw, formula = 'C16H12O5', fmz= -1) # keep fmz=-1 when you give a formula
# The isotopic pattern is obtained by the function of 
...
eics <- lapply(rawfiles, function(raw){
  getEIC(raw, c(0, Inf), c(288.05, 288.10))
})
